...
-- RUN THIS QUERY YOU'LL GET A LIST OF UPDATES, THEN RUNN THEM

SELECT 'UPDATE TB_QUERIES_VERSIONS SET DATASET_FK = '||datasetVersion.dataset_fk||', DATASET_VERSION_FK = null, STATUS = ''ACTIVE'' where id = '||queryVersion.id||';' 
...
SELECT 'UPDATE TB_QUERIES_VERSIONS SET STATUS = ''DISCONTINUED'' where id = '||queryVersion.id||';' 
FROM  tb_queries_versions queryVersion, 
      tb_datasets_versions datasetVersion,
      tb_stat_resources stat
WHERE queryversion.dataset_version_fk = datasetVersion.ID
