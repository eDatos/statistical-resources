...
scp -o ProxyCommand="ssh -W %h:%p deploy@estadisticas.arte-consultores.com" -r etc/deploy deploy@192.168.10.16m:$TRANSFER_PATH
scp -o ProxyCommand="ssh -W %h:%p deploy@estadisticas.arte-consultores.com" metamac-statistical-resources-web/target/statistical-resources-internal-*.war deploy@192.168.10.16m:$TRANSFER_PATH/statistical-resources-internal.war
scp -o ProxyCommand="ssh -W %h:%p deploy@estadisticas.arte-consultores.com" metamac-statistical-resources-external-web/target/statistical-resources-*.war deploy@192.168.10.16m:$TRANSFER_PATH/statistical-resources.war
ssh -o ProxyCommand="ssh -W %h:%p deploy@estadisticas.arte-consultores.com" deploy@192.168.10.16m <<EOF
