...
public class DatasetVersioningCopyUtils extends CommonVersioningCopyUtils {
    ...
    public static List<Mapping> createDatasourceMapping(DatasetVersion resource, DatasetVersion previous) {
        List<Mapping> datasourcesMapping = new ArrayList<>();
        for (int i = 0; i < resource.getDatasources().size(); i++) {
            datasourcesMapping
                    .add(new Mapping(previous.getDatasources().get(i).getIdentifiableStatisticalResource().getCode(), resource.getDatasources().get(i).getIdentifiableStatisticalResource().getCode()));
        }
